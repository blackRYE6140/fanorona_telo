...
class PatternRecognizer {
  ...
  static List<GridPosition> findThreatsToBlock(
    GameState state,
    Player opponent,
  ) {
  ...
  static ThreatAnalysis _analyzeLine(
    GameState state,
  ...
  static int _calculateDangerLevel(
    int opponentCount,
    int emptyCount,
    List<GridPosition> line,
  ) {
  ...
  static List<AIMove> findForkMoves(GameState state, Player player) {
    ...
    for (var piece in playerPieces) {
      ...
      for (var move in possibleMoves) {
        ...
        final threatsCreated = _countThreatsCreated(newState, player, move);
  ...
  static List<GridPosition> _getPossibleMovesForPiece(
    GameState state,
    GamePiece piece,
  ) {
  ...
  static GameState _simulateMove(
    GameState state,
    GamePiece piece,
  ...
  static int _countThreatsCreated(
    GameState newState,
    Player player,
    GridPosition movedPosition,
  ) {
    int threatCount = 0;

    for (var line in winningLines) {
      if (line.contains(movedPosition)) {
        final piecesOnLine = line
            .map((pos) => newState.getPieceAt(pos))
            .toList();
  ...
  static int _countPotentialThreats(GameState state, Player player) {
    ...
    for (var piece in playerPieces) {
      ...
      for (var move in possibleMoves) {
        final newState = _simulateMove(state, piece, move);
        if (_countThreatsCreated(newState, player, move) > 0) {
          threatCount++;
          break;
        }
      }
